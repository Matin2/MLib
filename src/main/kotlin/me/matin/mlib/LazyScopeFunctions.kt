...
val <T> Lazy<T>.nullable: T? get() = value.takeIf { isInitialized() }
 ...
 */
fun <T, R> Lazy<T>.lazyLet(block: (T) -> R): R? = block(value).takeIf { isInitialized() }

/**
 * Calls the specified function [block] with `this` value as its receiver if
 * it is initialized and returns it's result or null if `this` is not initialized.
 */
fun <T, R> Lazy<T>.lazyRun(block: T.() -> R): R? = block(value).takeIf { isInitialized() }